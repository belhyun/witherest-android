...
public class BoardContentActivity extends Activity implements ArticleServiceCallback{
	...
	private void initInstance() {
		...
		userType = i.getIntExtra("userType",-1);
		roomId = i.getIntExtra("roomId", -1);
		Log.v("유저 타입", userType + "   !!!");
	...
	private void initView() {
		...
		writerImageView.setImageResource(R.drawable.notice_profileimg_bg);
		String url = null;
	...
	private void updateUi(ReplyListResponseObject object) {
		...
		pageInfo = new PageInfo(roomId,page,object.totalBoardCount );
