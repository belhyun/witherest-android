...
public class SettingsActivity extends Activity implements OnClickListener, UserServiceCallback{
	...
	public void initListener(){
		...
		evernoteCheckBtn.setOnClickListener(this);
	}
	...
	public void onClick(View view) {
		...
		case R.id.activity_setting_evenote_container:
			if (ever.isLoggedin()){
				everNoteAction = EVERNOTE_REQUEST_LOGOUT;
		...
		case R.id.activity_setting_evernote_btn:
			everNoteAction = evernoteCheckBtn.isChecked()? EVERNOTE_LOGINSTATE_REQUEST_RECORING
					: EVERNOTE_LOGINSTATE_REQUEST_NO_RECORDING;
			updateEvenoteConfig(evernoteCheckBtn.isChecked());
		}
	...
	public void onUserServiceCallback(BaseResponseObject object) {
		...
		if (object.resultCode == Service.RESULT_FAIL) {
			if (object.requestType == Service.REQUEST_TYPE_ALARM_UPDATE){
				if (everNoteAction == EVERNOTE_REQUEST_LOGOUT ){
					Session.getInstance().user.setIsEvernote(!Session.getInstance().user.isEvernote);
					everNoteAction = -1;
				}else {
					evernoteCheckBtn.setChecked(!evernoteCheckBtn.isChecked());
					Session.getInstance().user.setIsEvernote(evernoteCheckBtn.isChecked());
					everNoteAction = -1;
				}
			}
			return;
