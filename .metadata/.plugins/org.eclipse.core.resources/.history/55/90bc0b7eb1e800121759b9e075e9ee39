...
public class Fragment_MyCheck extends Fragment implements RoomServiceCallback{
	...
	public int curCheckActionRoomId= -1;
	...
	public void onRoomServiceCallback(BaseResponseObject object) {
		...
		case Service.REQUEST_TYPE_GET_MY_CHECKROOMS:
			...
			if (MainActivity.isMyCheckRoomMustReloaded == true){
				MainActivity.isMyCheckRoomMustReloaded = false;
			}
			updateUi();
			setAdapter();
			break;
				...
				builder2.setPositiveButton(R.string.confirm,
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int which) {
								new Handler().postDelayed(new Runnable() {
									public void run() {
										if (curCheckActionRoomId != -1){
											checkRoom(curCheckActionRoomId);
										}
									}
								}, 100);
							}
						...
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int which) {
								new Handler().postDelayed(new Runnable() {
									public void run() {
										if (curCheckActionRoomId != -1){
											cancelCheck(curCheckActionRoomId);
										}
									}
								}, 100);
							}
	...
	public void onResume() {
		...
		if (myCheckResponse == null || MainActivity.isMyCheckRoomMustReloaded == true){
			loadMyCheckRooms();
		}else {
			updateUi();
			setAdapter();
	...
	public void updateUi(){
		if (!CommonUtils.isNullOrZeroOrEmpty(Session.getInstance().user.nickName)){
			nameView.setText(Session.getInstance().user.nickName);
		}
		
		if (!CommonUtils.isNullOrZeroOrEmpty(Session.getInstance().user.purpose)){
			purposeView.setText(Session.getInstance().user.purpose);
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
	    	...
	    	viewHolder.myCheck.setOnClickListener(new OnClickListener() {
				public void onClick(View view) {
					...
					refreshPos = pos;
					refeshBadge = cBadge;
					curCheckActionRoomId = roomList.get(pos).roomId;
