...
/*	public void sendGet(String url, String paramEncoding, String responseEncoding){
	...
	public String sendMultipartEx(String url, String filename, Map<String, String> paramMap){
		...
		try {
			...
			Log.v("SocketException[multipart] ", e.toString());
			
		}catch(SocketTimeoutException e){
			// 연결이 된 후 읽기 제한시간을 초과했을 때 발생한다.
			Log.v("SocketTimeoutException[multipart] ", e.toString());
			result = ErrorHandler.READ_TIMEOUT_STIRNG;
			
		} catch (ClientProtocolException e) {
			Log.v("ClientProtocolException[multipart]", e.toString());
			...
			*/
			...
			Log.v("RuntimeException[multipart]",e.getMessage());
			httpPost.abort();	
		}catch(Exception e){
			Log.v("Exception[multipart]!!", e.toString());
		} finally {
	...
	public void finishedBackgroundProcess(String requsetUrl,String responseText, String requestCode) {
		...
		if (CommonUtils.isNullOrEmpty(responseText)){
			obj = new BaseResponseObject(ErrorHandler.COMMON_ERROR, null, CommonUtils.str2int(requestCode));
		}
		
		else if (responseText.equals(ErrorHandler.COMMON_ERROR_STRING)){
			obj = new BaseResponseObject(ErrorHandler.COMMON_ERROR, null, CommonUtils.str2int(requestCode));
		}
		
		else if (responseText.equals(ErrorHandler.NETWORK_DISABLE_STIRNG)){
			obj = new BaseResponseObject(ErrorHandler.NETWORK_DISABLE, null, CommonUtils.str2int(requestCode));
		}
		
		else if (responseText.equals(ErrorHandler.READ_TIMEOUT_STIRNG)){
			obj = new BaseResponseObject(ErrorHandler.READ_TIMEOUT, null, CommonUtils.str2int(requestCode));
		}
		
		else if (responseText.equals(ErrorHandler.CONNECTION_TIMEOUT_STIRNG)){
			obj = new BaseResponseObject(ErrorHandler.CONNECTION_TIMEOUT, null, CommonUtils.str2int(requestCode));
		}
		
		else {
			obj = ParsingService.parseJson(responseText);
			if (obj == null){
				obj = new BaseResponseObject(ErrorHandler.PARSING_ERROR,null,CommonUtils.str2int(requestCode));
			}
		}
		...
		Log.v("response_text", responseText != null? responseText:"");
	}
